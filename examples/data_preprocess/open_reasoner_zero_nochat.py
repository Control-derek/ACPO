...
# aime25
aime_ds1 = load_dataset("opencompass/AIME2025", "AIME2025-I")
aime_ds2 = load_dataset("opencompass/AIME2025", "AIME2025-II")
aime_ds = concatenate_datasets([aime_ds1["test"], aime_ds2["test"]])
eval_parsed_list_aime25 = []
for data_i, one_data in enumerate(aime_ds):
    answer = one_data["answer"].strip()
    problem = one_data["question"].strip()
    parsed_entry = copy.deepcopy(entry_template)
    parsed_entry["data_source"] = "aime2025"
    parsed_entry["prompt"] = str(format_prompt(problem))
    parsed_entry["reward_model"]["ground_truth"] = str(answer)
    parsed_entry["extra_info"]["index"] = data_i
    parsed_entry["extra_info"]["split"] = "test"
    eval_parsed_list_aime25.append(parsed_entry)
test_dataset_aime25 = datasets.Dataset.from_list(eval_parsed_list_aime25)
test_dataset_aime25.to_parquet(os.path.join(local_dir, 'test_aime2025.parquet'))