        ...
        if peft_config:
            if self.base_sync_done:
                ...
                def replace_lora_wrapper(k):
                    # "embed_tokens" is the workaround for llama
                    # https://github.com/volcengine/verl/issues/2160
                    if getattr(self.model_config, "model_type", None) == "llama":
                        stacked_params = ["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj", "embed_tokens"]
                    else:
