...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if len(self.config.model.freeze_layer_ids) > 0:
                assert self._is_lora is False
                print("Freeze weight update for:", self.config.model.freeze_layer_ids)
                for layer_id in self.config.model.freeze_layer_ids:
                    for name, parameter in actor_module.model.layers[layer_id].named_parameters():
        ...
        if role == "actor" and optim_config is not None:
            ...
            elif optim_config["type"] == "SGD_momentum":
                actor_optimizer = optim.SGD(
                    actor_module_fsdp.parameters(),
                    lr=optim_config.lr,
